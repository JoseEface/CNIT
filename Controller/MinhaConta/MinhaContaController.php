...
try
{
    case "AlterarPerfil":
        if(isset($_SESSION[\Controller\Acesso\LoginDefinitions::SESSION_LOGIN_VAR]))
        {
            ...
            $senhaConfirma = flter_input(INPUT_POST,"senha",\FILTER_SANITIZE_STRING);
            ...
            if(empty($senhaAtual) && (!empty($senhaNova) || !empty($senhaConfirma)))
                throw new \RuntimeException("AlterarPerfil: é necessário definir todas as senhas caso deseje trocar");
    ...
    case "SenhaConfere":
        ...
        if(!isses($_SESSION[\Controller\Acesso\LoginDefinitions::SESSION_LOGIN_VAR]))
